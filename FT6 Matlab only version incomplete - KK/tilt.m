...
% MAX: at distance 5000mm = 20% * 5000mm = 1000mm 
% MAX: at distance 650mm = 20% * 650mm = 130mm 
% MIN: 0
...
if nframes < 50
    ...
    for j = 1:nframes
        ...
        if isempty(sorted)
            diff_tilt(j) = 999999; % fails images with no valid data
        elseif size(sorted,1) <= spray_high(j)
            diff_tilt(j) = 888888; % fails images with all data points are spray
        else               
